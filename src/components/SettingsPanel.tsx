  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <GlassCard className="space-y-4">
        ...
        <div className="pt-2">
          <p className="text-xs uppercase tracking-[0.3em] text-ink-300">Create folder</p>
          <div className="mt-3 flex flex-col gap-3">
            <input
              value={name}
              onChange={(event) => setName(event.target.value)}
              placeholder="Folder name"
              className="bg-ink-800/40 border border-white/10 rounded-xl px-3 py-2 text-sm outline-none"
            />
      ...
      <GlassCard className="lg:col-span-2 space-y-4">
        <h3 className="text-lg font-semibold">Folder controls</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {folders.map((folder) => (
            <div key={folder.name} className="p-4 rounded-xl border border-white/10 bg-white/5">
              <h4 className="font-semibold truncate">{folder.name}</h4>
              <div className="mt-3 flex items-center gap-3 text-xs text-ink-200">
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
